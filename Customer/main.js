...
document.getElementById("confirmOrderBtn")?.addEventListener("click", () => {
  ...
  document.querySelectorAll('[data-category-id]').forEach((link) => {
    link.addEventListener('click', function (e) {
      ...
      setActive(this);
      fetchProducts(
        `http://localhost:3000/products?categoryId=${this.dataset.categoryId}`,
      );
    });
  });
    ...
    link.addEventListener('click', function (e) {
      ...
      switch (this.dataset.priceId) {
        case 'cat_007':
          ...
          break;
        case 'cat_008':
          url += '?price_gte=100&price_lte=500';
          break;
        case 'cat_009':
          url += '?price_gte=501&price_lte=1000';
          break;
        case 'cat_0010':
          url += '?price_gte=1001';
          break;
  ...
  if (wishlistGrid) {
    function renderWishlist() {
      const wishlist = getWishlist().filter(
        (p) => p && p.id && p.name && p.price && p.imageUrl,
      );
      wishlistGrid.innerHTML =
        wishlist.length === 0
